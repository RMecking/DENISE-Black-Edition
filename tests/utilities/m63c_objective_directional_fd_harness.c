...
static double direction_value(int j, int i) {
    double pattern = 0.11 + 0.23 * sin(0.41 * i + 0.19 * j)
                     - 0.14 * cos(0.27 * i - 0.33 * j);
    return base_mu_value(j, i) * pattern;
}

static void prepare_model(
        double epsilon, int truth, const struct q_tau_mapping *mapping,
        struct field *primary, struct field *rho, struct field *q,
    ...
    for (j = 1; j <= NY; ++j) {
        for (i = 1; i <= NX; ++i) {
            double base = base_mu_value(j, i);
            double truth_delta = base * (0.026 * cos(0.22 * i + 0.37 * j)
                                         - 0.012 * sin(0.35 * i - 0.16 * j));
            primary->v[j][i] = (float)(base + (truth ? truth_delta :
                                               epsilon * direction_value(j, i)));
            rho->v[j][i] = (float)(2130.0 + 1.8 * i + 1.1 * j
                                   + 4.0 * sin(0.21 * i + 0.09 * j));
            q->v[j][i] = (float)(31.0 + 0.19 * i + 0.14 * j);
...
int main(int argc, char **argv) {
    ...
    struct field rhoi, fipjp, f, primary, rho, q, tau, mu_x, mu_y;
    struct field tau_x, tau_y, grad_mu, grad_rho, grad_q, diag[6], dummy;
...
#define FIELD(name) name = field_new(-h, NY + h + 1, 1 - h, NX + h)
    ...
    FIELD(tau); FIELD(mu_x); FIELD(mu_y); FIELD(tau_x); FIELD(tau_y);
    FIELD(grad_mu); FIELD(grad_rho); FIELD(grad_q); FIELD(dummy);
...
#undef FIELD
    ...
    init_q_tau_mapping(&mapping, Q_PARAMETERIZATION_PHYSICAL, L,
    ...
    prepare_model(0.0, 1, &mapping, &primary, &rho, &q, &tau, &rhoi,
    ...
    prepare_model(0.0, 0, &mapping, &primary, &rho, &q, &tau, &rhoi,
                  &mu_x, &mu_y, &tau_x, &tau_y, &fipjp, &f, &dip, &d,
                  eta, bip, bjm, reference_sum);
    ...
    material.eta_y = eta; material.invmat1 = 3; material.mapping = &mapping;
    material.primary_post = primary.v; material.rho_post = rho.v;
    material.owned_q = q.v; material.grad_primary = grad_mu.v;
    ...
    if (status != 0) MPI_Abort(MPI_COMM_WORLD, 905);
    for (j = 1; j <= NY; ++j) {
        for (i = 1; i <= NX; ++i) {
            double direction = direction_value(j, i);
            d_ad += grad_mu.v[j][i] * direction;
    ...
    for (e = 0; e < 4; ++e) {
        ...
        prepare_model(ep, 0, &mapping, &primary, &rho, &q, &tau, &rhoi,
                      &mu_x, &mu_y, &tau_x, &tau_y, &fipjp, &f, &dip, &d,
                      eta, bip, bjm, reference_sum);
        ...
        printf("{\"epsilon\":%.17g,\"J_plus\":%.17g,\"J_minus\":%.17g,"
