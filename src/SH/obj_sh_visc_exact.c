/* Inactive exact viscoelastic SH multi-shot objective-only wrapper. */

#include "fd.h"

static int exact_objective_multi_preflight(
...
static void exact_objective_multi_map_observed(
...
static void exact_objective_multi_release_dynamic_receivers(
        const struct visco_sh_exact_multi_shot_request *request,
...
int visco_sh_exact_objective(
    ...
    extern FILE *FP;
    struct visco_sh_exact_objective_shot_request shot_request;
    struct visco_sh_exact_objective_shot_result shot_result;
    ...
    for (ishot = 1; ishot <= nshots; ++ishot) {
        ...
        exact_objective_multi_map_observed(
                request->legacy_fwi_seismogram, request->acquisition,
                nrec_local, request->ns);
        ...
        shot_status = visco_sh_exact_objective_shot(
                &shot_request, &shot_result);
        if (shot_status == 0) objective += shot_result.objective;
        ...
        if (READREC == 2)
            exact_objective_multi_release_dynamic_receivers(
                    request, nrec_local, nrec_global);
        if (shot_status != 0) return shot_status;
    }
