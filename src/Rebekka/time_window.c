...
err(" picks_?.dat could not be opened !");
  ...
  for(i=1;i<=ntr_glob;i++){
    ...
    pick_tmp1[i] = dump + TWLENGTH_MINUS; /* geändert Rebekka 01.09.2021*/
  }
...
err(" picks_?.dat could not be opened !");
  ...
  for(i=1;i<=ntr;i++){
    ...
    /*picked_times1[i] = dump; auskommentiert Rebekka, 01.09.2021*/
    /* ab hier geändert: Rebekka, 01.09.2021 */
    picked_times1[i] = dump - TWLENGTH_MINUS;
  }
...
for(i=1;i<=ntr;i++){
      for(j=1;j<=ns;j++){
         ...
         /*taper1 = exp(-TWLENGTH_MINUS*dump*dump); auskommentiert Rebekka, 01.09.2021*/
	 ...
	 if(TIMEWIN==1){
           ...
           } /* einkommentiert: Rebekka, 01.09.2021*/    
